...
import { X, ChevronDown, ChevronRight, Clock, AlertCircle, CheckCircle2, Loader2 } from 'lucide-react'
...
const STATUS_CONFIG: Record<string, { color: string; bg: string; icon: typeof CheckCircle2; label: string }> = {
    completed: { color: '#10b981', bg: 'rgba(16,185,129,0.1)', icon: CheckCircle2, label: 'Completed' },
    ...
    pending: { color: 'rgba(255,255,255,0.3)', bg: 'rgba(255,255,255,0.03)', icon: Clock, label: 'Pending' },
    skipped: { color: 'rgba(255,255,255,0.2)', bg: 'rgba(255,255,255,0.02)', icon: Clock, label: 'Skipped' }
}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div
                        style={{
                            ...
                            height: 6,
                            borderRadius: '50%',
                            background: isRunning ? '#f59e0b' : logs.some((l) => l.status === 'failed') ? '#ef4444' : '#10b981',
                            boxShadow: `0 0 8px ${isRunning ? '#f59e0b' : logs.some((l) => l.status === 'failed') ? '#ef4444' : '#10b981'}`
                        }}
                    />
                    <span style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.8)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
                        Execution Log
                    </span>
                    ...
                    return (
                        <div key={log.nodeId} style={{ marginBottom: 4 }}>
                            ...
                            >
                                ...
                                {/* Node name */}
                                <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.85)', fontWeight: 500, flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                    {log.nodeId}
                                </span>
                                ...
                                >
                                    ...
                                    {log.error && (
                                        <div style={{ marginBottom: 8 }}>
                                            ...
                                            <span style={{ color: 'rgba(255,255,255,0.7)', fontFamily: 'monospace', fontSize: 10 }}>
                                                {log.error}
                                            </span>
                                        </div>
                                    )}
                                    ...
                                    <div style={{ display: 'flex', gap: 16, color: 'rgba(255,255,255,0.4)' }}>
                                        ...
                                        {log.endedAt && <span>Ended: {new Date(log.endedAt).toLocaleTimeString()}</span>}
                                    ...
                                    {log.output && Object.keys(log.output).length > 0 && (
                                        <div style={{ marginTop: 8 }}>
                                            <span style={{ color: 'rgba(255,255,255,0.5)', fontWeight: 600, display: 'block', marginBottom: 4 }}>
                                                Output:
                                            </span>
                                            <pre
                                                style={{
