...
function buildAcceptString(formats: string[]): string {
  ...
  return formats.map((f) => mimeMap[f.toLowerCase()] ?? `.${f}`).join(',')
}
...
export default function IngestUploadZone({ acceptedFormats }: IngestUploadZoneProps): ReactElement {
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <strong style={{ color: '#60a5fa' }}>Click to browse</strong> or drag files here
        </div>
        <div
          style={{
            fontSize: 9,
