...
function TransformOperationStack({
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            {operations.map((op, idx) => {
                ...
                return (
                    ...
                    >
                        ...
                        >
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                <button
                                    ...
                                    style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', cursor: 'pointer', padding: 2, opacity: idx === 0 ? 0.3 : 1 }}
                                    disabled={idx === 0}
                                >
                                    <ChevronUp size={10} />
                                </button>
                                <button
                                    ...
                                    style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', cursor: 'pointer', padding: 2, opacity: idx === operations.length - 1 ? 0.3 : 1 }}
                                    disabled={idx === operations.length - 1}
                                >
                                    <ChevronDown size={10} />
                                </button>
                                <button
                                    onClick={(e) => { e.stopPropagation(); removeOperation(idx) }}
                                    style={{ background: 'none', border: 'none', color: 'rgba(239,68,68,0.6)', cursor: 'pointer', padding: 2 }}
                                >
                                    <Trash2 size={10} />
                                </button>
                            </div>
                        ...
                        {/* Expanded fields */}
                        {isExpanded && (
                            <div style={{ padding: '6px 8px 8px', display: 'flex', flexDirection: 'column', gap: 6, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            >
                <option value="" style={{ background: '#1a1a2e' }}>+ Add Operation</option>
...
function renderOpFields(
    ...
    switch (op.type) {
        case 'rename':
            return (
                <>
                    <MiniField label="From field" value={String(op.from ?? '')} onChange={(v) => update('from', v)} placeholder="original_name" />
                    <MiniField label="To field" value={String(op.to ?? '')} onChange={(v) => update('to', v)} placeholder="new_name" />
                </>
            )
        case 'filter':
            return (
                <MiniField
                    ...
                    value={Array.isArray(op.fields) ? (op.fields as string[]).join(', ') : String(op.fields ?? '')}
                    onChange={(v) => update('fields', v.split(',').map((s) => s.trim()).filter(Boolean))}
        ...
        case 'default':
            return (
                <>
                    <MiniField label="Field name" value={String(op.field ?? '')} onChange={(v) => update('field', v)} placeholder="field_name" />
                    <MiniField label="Default value" value={String(op.value ?? '')} onChange={(v) => update('value', v)} placeholder="default_value" />
                </>
            )
        case 'coerce':
            return (
        ...
        case 'format':
            return (
                <>
                    <MiniField label="Field name" value={String(op.field ?? '')} onChange={(v) => update('field', v)} placeholder="field_name" />
                    <MiniField label="Template" value={String(op.template ?? '')} onChange={(v) => update('template', v)} placeholder="{{value}} USD" />
                </>
            )
        case 'concat':
            return (
                <>
                    ...
                    />
                    <MiniField label="Separator" value={String(op.separator ?? ' ')} onChange={(v) => update('separator', v)} placeholder=" " />
                    <MiniField label="Output field" value={String(op.outputField ?? '')} onChange={(v) => update('outputField', v)} placeholder="full_name" />
                </>
            )
        case 'compute':
            return (
                <>
                    <MiniField label="Expression" value={String(op.expression ?? '')} onChange={(v) => update('expression', v)} placeholder="price * quantity" />
                    <MiniField label="Output field" value={String(op.outputField ?? '')} onChange={(v) => update('outputField', v)} placeholder="total" />
                </>
            )
        default:
            return <MiniField label="Type" value={op.type} onChange={(v) => update('type', v)} placeholder="operation type" />
    }
}
                        ...
                        <TransformOperationStack
                            operations={
                                Array.isArray(config.operations)
                                    ? (config.operations as TransformOp[])
                                    : []
                            }
